...
protected:
    ...
    void paintGL() override {
        ...
        const QVector3D eye(m_b.center[0] + m_dist * cp * sy, m_b.center[1] - m_dist * cp * cy,
    ...
    void keyPressEvent(QKeyEvent *e) override {
        if (e->key() == Qt::Key_1) m_pointSize = 1.5f;
        if (e->key() == Qt::Key_2) m_pointSize = 3.0f;
        if (e->key() == Qt::Key_3) m_pointSize = 5.0f;
        update();
    }
...
int main(int argc, char *argv[]) {
    ...
    if (argc > 1) {
        ...
        const bool isLas = path.endsWith(QStringLiteral(".las"), Qt::CaseInsensitive) ||
                           path.endsWith(QStringLiteral(".laz"), Qt::CaseInsensitive);
        std::string lerr;
        const bool ok = isLas ? load_las(pc, argv[1], &lerr) : load_ply(pc, argv[1]);
        if (!ok) {
            // Surface WHY the load failed instead of silently swapping in the demo
            // scan — a user pointing this at a real scan needs to know their file
            // was rejected, not see an unrelated synthetic street.
            std::fprintf(stderr, "aurum-pointcloud-viewer: failed to load %s%s%s\n", argv[1],
                         lerr.empty() ? "" : ": ", lerr.c_str());
            std::fprintf(stderr, "aurum-pointcloud-viewer: falling back to the demo scan\n");
