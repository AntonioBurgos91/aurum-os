...
int main(int argc, char *argv[]) {
    ...
    const QString arg = argc > 2 ? QString::fromUtf8(argv[2]) : QStringLiteral("road");
    if (arg == "road") {
        ...
        std::string lerr;
        const bool isLas = arg.endsWith(QStringLiteral(".las"), Qt::CaseInsensitive);
        const bool ok = isLas ? load_las(pc, argv[2], &lerr) : load_ply(pc, argv[2]);
        if (!ok) {
    ...
    if (!anyLabel) mode = GLRenderer::ColorMode::Defect;
    ...
    view.lookAt(
        QVector3D(b.center[0] - dist * 0.7f, b.center[1] - dist * 0.9f, b.center[2] + dist * 0.35f),
        QVector3D(b.center[0], b.center[1], b.center[2] - b.radius * 0.05f), QVector3D(0, 0, 1));
