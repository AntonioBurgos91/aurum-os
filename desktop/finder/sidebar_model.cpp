...
void SidebarModel::rebuild() {
    ...
    const auto add_fav = [&](const QString& name, const QString& path, const QString& icon) {
        if (exists(path))
            m_rows.push_back({name, path, icon, "Favorites"});
    };
    add_fav("Home",     QDir::homePath(),                                   "user-home");
    add_fav("Desktop",  QStandardPaths::writableLocation(QStandardPaths::DesktopLocation),  "user-desktop");
    add_fav("Documents",QStandardPaths::writableLocation(QStandardPaths::DocumentsLocation),"folder-documents");
    add_fav("Downloads",QStandardPaths::writableLocation(QStandardPaths::DownloadLocation), "folder-download");
    add_fav("Pictures", QStandardPaths::writableLocation(QStandardPaths::PicturesLocation), "folder-pictures");
    ...
    const auto add_ml = [&](const QString& name, const QString& path, const QString& icon) {
        if (exists(path))
            m_rows.push_back({name, path, icon, "ML"});
    };
