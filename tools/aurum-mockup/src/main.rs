...
fn draw_wallpaper(pm: &mut Pixmap) {
    ...
    for i in 0..40 {
        ...
        fill_round_rect(pm, glow_cx - r, glow_cy - r, r * 2.0, r * 2.0, r, rgba8(255, 244, 220, a));
    }
    ...
    for _ in 0..300 {
        ...
        fill_round_rect(pm, x, y, r, r, r * 0.5, rgba8(brightness, brightness, (brightness as u32 + 30).min(255) as u8, 220));
    }
...
fn draw_menubar(pm: &mut Pixmap, fonts: &FontStack) {
    ...
    paint.anti_alias = true;
    pm.fill_rect(rect(0.0, 0.0, W as f32, bar_h), &paint, Transform::identity(), None);

    // Bottom hairline.
    let mut hair = Paint::default();
    hair.set_color(rgba8(255, 255, 255, 25));
    pm.fill_rect(rect(0.0, bar_h - 1.0, W as f32, 1.0), &hair, Transform::identity(), None);
    ...
    let app_name = "Finder";
    draw_text(pm, &fonts.sans_bold, app_name, pen, bar_h / 2.0 + 9.0, 26.0, c(TEXT_PRI));
    pen += text_width(&fonts.sans_bold, app_name, 26.0) + 30.0;

    for label in ["File", "Edit", "View", "Go", "Window", "Help"] {
        draw_text(pm, &fonts.sans, label, pen, bar_h / 2.0 + 9.0, 24.0, c(TEXT_PRI));
        pen += text_width(&fonts.sans, label, 24.0) + 28.0;
    }

    // --- Right: DL telemetry + clock ---
    struct Stat<'a> { label: &'a str, value: &'a str, tone: Rgba }
    let stats = [
        Stat { label: "GPU",  value: "78%",      tone: WARNING },
        Stat { label: "VRAM", value: "21.6/24G", tone: TEXT_PRI },
        Stat { label: "°C",   value: "72",       tone: WARNING },
        Stat { label: "NET",  value: "1.2 GB/s", tone: TEXT_PRI },
        Stat { label: "JOBS", value: "3",        tone: ACCENT },
    ];
    ...
    for s in &stats {
        draw_text(pm, &fonts.sans_bold, s.label, rx, bar_h / 2.0 + 7.0, label_size, c(TEXT_SEC));
        rx += text_width(&fonts.sans_bold, s.label, label_size) + 8.0;
        draw_text(pm, &fonts.sans, s.value, rx, bar_h / 2.0 + 9.0, stat_size, c(s.tone));
        rx += text_width(&fonts.sans, s.value, stat_size) + stat_gap;
    }
    draw_text(pm, &fonts.sans, clock, rx + 16.0, bar_h / 2.0 + 9.0, stat_size, c(TEXT_PRI));
}
...
fn draw_dock(pm: &mut Pixmap, fonts: &FontStack) {
    ...
    let peak = 1.6;
    let sigma_idx = 1.5;
    let scales: Vec<f32> = (0..icons.len()).map(|i| {
        let d = i as f32 - 3.5;
        1.0 + (peak - 1.0) * (-(d * d) / (sigma_idx * sigma_idx)).exp()
    }).collect();

    let total_w: f32 = scales.iter().map(|s| s * icon_size).sum::<f32>()
                       + (n - 1.0) * icon_gap;
    let badge_w = 230.0;
    let divider_w = 1.0;
    ...
    fill_round_rect(pm, shelf_x, shelf_y, shelf_w, shelf_h, 26.0, rgba8(28, 28, 32, 230));
    stroke_round_rect(pm, shelf_x, shelf_y, shelf_w, shelf_h, 26.0, rgba8(255, 255, 255, 38), 1.0);
    let mut highlight = Paint::default();
    highlight.set_color(rgba8(255, 255, 255, 14));
    pm.fill_rect(rect(shelf_x + 18.0, shelf_y + 6.0, shelf_w - 36.0, 14.0), &highlight, Transform::identity(), None);
    ...
    for (i, ch) in icons.iter().enumerate() {
        ...
        pm.fill_rect(rect(x + sz * 0.10, iy + sz * 0.08, sz * 0.80, sz * 0.18), &hp, Transform::identity(), None);
        let glyph_size = sz * 0.55;
        let gw = text_width(&fonts.sans_bold, ch, glyph_size);
        draw_text(
            pm, &fonts.sans_bold, ch,
            x + (sz - gw) / 2.0,
            iy + sz * 0.7,
            glyph_size,
            rgba8(0, 0, 0, 220),
    ...
    pm.fill_rect(rect(div_x, shelf_y + 22.0, divider_w, shelf_h - 44.0), &dp, Transform::identity(), None);
    ...
    draw_text(pm, &fonts.sans_bold, "GPU", bx + 22.0, by + 38.0, 26.0, c(TEXT_SEC));
    draw_text(pm, &fonts.sans_bold, "78%", bx + 22.0, by + 96.0, 56.0, c(WARNING));

    // Spark line.
    let mut sp = PathBuilder::new();
    let sx = bx + 22.0;
    let sy = by + bh - 20.0;
    let pts = [0.25f32, 0.35, 0.30, 0.45, 0.55, 0.40, 0.60, 0.78, 0.65, 0.72, 0.78];
    ...
    if let Some(path) = sp.finish() {
        ...
        pm.stroke_path(&path, &sp_paint, &Stroke {
            width: 3.0, line_cap: LineCap::Round, line_join: LineJoin::Round, ..Default::default()
...
fn draw_finder(pm: &mut Pixmap, fonts: &FontStack) {
    ...
    pm.fill_rect(rect(x, y + tb_h - 1.0, w, 1.0), &hairp, Transform::identity(), None);
    ...
    draw_text(pm, &fonts.sans_bold, "<", x + 130.0, y + tb_h / 2.0 + 10.0, 26.0, c(TEXT_PRI));
    draw_text(pm, &fonts.sans_bold, ">", x + 170.0, y + tb_h / 2.0 + 10.0, 26.0, c(TEXT_DIM));
    draw_text(pm, &fonts.sans_bold, "/home/abb/projects/llm-finetune/", x + 230.0, y + tb_h / 2.0 + 8.0, 22.0, c(TEXT_PRI));

    let sf_w = 320.0;
    let sf_x = x + w - sf_w - 24.0;
    fill_round_rect(pm, sf_x, y + 16.0, sf_w, tb_h - 32.0, 8.0, rgba8(48, 48, 54, 230));
    draw_text(pm, &fonts.sans, "Search...", sf_x + 14.0, y + tb_h / 2.0 + 7.0, 20.0, c(TEXT_DIM));
    ...
    pm.fill_rect(rect(x, body_y, side_w, body_h), &sb_paint, Transform::identity(), None);

    draw_text(pm, &fonts.sans_bold, "FAVORITES", x + 26.0, body_y + 40.0, 16.0, c(TEXT_SEC));
    let sb_items = [
        ("Recents",   false),
        ("Home",      false),
        ("Projects",  true),
    ...
    for (txt, selected) in &sb_items {
        if *selected {
            fill_round_rect(pm, x + 14.0, iy - 22.0, side_w - 28.0, 32.0, 8.0, rgba8(76, 134, 215, 220));
        }
        // Tiny chip in front of label.
        let chip_col = if *selected { rgba8(255, 255, 255, 220) } else { rgba8(160, 160, 170, 200) };
        fill_round_rect(pm, x + 28.0, iy - 16.0, 18.0, 18.0, 4.0, chip_col);
        draw_text(pm, &fonts.sans, txt, x + 56.0, iy, 22.0, c(TEXT_PRI));
        iy += 44.0;
    }
    draw_text(pm, &fonts.sans_bold, "ML PROJECTS", x + 26.0, body_y + 460.0, 16.0, c(TEXT_SEC));
    let mlp = ["llm-finetune", "vision-transformer", "timeseries-mlp", "diffusion-toy"];
    iy = body_y + 500.0;
    for txt in &mlp {
        fill_round_rect(pm, x + 28.0, iy - 16.0, 18.0, 18.0, 4.0, rgba8(160, 160, 170, 200));
        draw_text(pm, &fonts.sans, txt, x + 56.0, iy, 22.0, c(TEXT_PRI));
        iy += 40.0;
    }
    ...
    pm.fill_rect(rect(x + side_w, body_y, 1.0, body_h), &div, Transform::identity(), None);

    // File list header.
    let lx = x + side_w;
    draw_text(pm, &fonts.sans_bold, "Name",     lx + 24.0,           body_y + 40.0, 18.0, c(TEXT_SEC));
    draw_text(pm, &fonts.sans_bold, "Modified", lx + list_w - 360.0, body_y + 40.0, 18.0, c(TEXT_SEC));
    draw_text(pm, &fonts.sans_bold, "Size",     lx + list_w - 110.0, body_y + 40.0, 18.0, c(TEXT_SEC));
    pm.fill_rect(rect(lx + 14.0, body_y + 56.0, list_w - 28.0, 1.0), &div, Transform::identity(), None);

    struct Row<'a> { kind: char, name: &'a str, modified: &'a str, size: &'a str, selected: bool }
    let rows = [
        Row { kind: 'N', name: "01_data_exploration.ipynb",   modified: "Today  09:14",     size: "412 KB", selected: false },
        Row { kind: 'N', name: "02_tokenizer_choice.ipynb",   modified: "Today  09:51",     size: "188 KB", selected: false },
        Row { kind: 'N', name: "03_finetune_lora.ipynb",      modified: "Today  10:02",     size: "612 KB", selected: true  },
        Row { kind: 'M', name: "checkpoint-step-2400.safetensors", modified: "Today  10:14", size: "13.2 GB", selected: false },
        Row { kind: 'M', name: "checkpoint-step-2000.safetensors", modified: "Today  09:38", size: "13.2 GB", selected: false },
        Row { kind: 'D', name: "train_split.parquet",         modified: "Yesterday 16:22", size: "1.8 GB", selected: false },
        Row { kind: 'D', name: "val_split.parquet",           modified: "Yesterday 16:22", size: "204 MB", selected: false },
        Row { kind: 'C', name: "config.yaml",                 modified: "Mon  21:08",       size: "2.4 KB", selected: false },
        Row { kind: 'P', name: "train.py",                    modified: "Mon  21:08",       size: "8.7 KB", selected: false },
        Row { kind: 'P', name: "evaluate.py",                 modified: "Mon  21:08",       size: "4.3 KB", selected: false },
        Row { kind: 'C', name: "requirements.txt",            modified: "Mon  21:08",       size: "612 B",  selected: false },
        Row { kind: 'F', name: "logs/",                       modified: "Today  10:14",     size: "—",      selected: false },
    ];
    let mut ry = body_y + 90.0;
    for r in &rows {
        if r.selected {
            fill_round_rect(pm, lx + 12.0, ry - 24.0, list_w - 24.0, 38.0, 8.0, rgba8(76, 134, 215, 220));
        }
        ...
        draw_text(pm, &fonts.sans_bold, lbl, lx + 27.0, ry - 1.0, 14.0, rgba8(0, 0, 0, 220));
        draw_text(pm, &fonts.sans, r.name, lx + 62.0, ry, 22.0, c(TEXT_PRI));
        let c2 = if r.selected { c(TEXT_PRI) } else { c(TEXT_SEC) };
        draw_text(pm, &fonts.sans, r.modified, lx + list_w - 360.0, ry, 21.0, c2);
        draw_text(pm, &fonts.sans, r.size,     lx + list_w - 110.0, ry, 21.0, c2);
        ry += 46.0;
    }

    pm.fill_rect(rect(x + side_w + list_w, body_y, 1.0, body_h), &div, Transform::identity(), None);

    // Preview pane.
    let px = x + side_w + list_w;
    draw_text(pm, &fonts.sans_bold, "QUICK LOOK", px + 24.0, body_y + 22.0, 16.0, c(TEXT_SEC));

    fill_round_rect(pm, px + 24.0, body_y + 40.0, prev_w - 48.0, 240.0, 14.0, rgba8(38, 38, 44, 240));
    draw_text(pm, &fonts.sans_bold, "03_finetune_lora.ipynb", px + 38.0, body_y + 76.0, 20.0, c(TEXT_PRI));
    draw_text(pm, &fonts.sans, "Jupyter  ·  28 cells  ·  Python 3.11", px + 38.0, body_y + 104.0, 18.0, c(TEXT_SEC));
    draw_text(pm, &fonts.sans_bold, "In [12]:", px + 38.0, body_y + 148.0, 18.0, c(ACCENT));
    draw_text(pm, &fonts.sans, "trainer = SFTTrainer(",     px + 130.0, body_y + 148.0, 18.0, c(TEXT_PRI));
    draw_text(pm, &fonts.sans, "    model=model,",           px + 130.0, body_y + 172.0, 18.0, c(TEXT_PRI));
    draw_text(pm, &fonts.sans, "    args=TrainingArguments(...),", px + 130.0, body_y + 196.0, 18.0, c(TEXT_PRI));
    draw_text(pm, &fonts.sans, ")", px + 130.0, body_y + 220.0, 18.0, c(TEXT_PRI));
    draw_text(pm, &fonts.sans, "trainer.train()", px + 130.0, body_y + 244.0, 18.0, c(TEXT_PRI));

    // Metadata box.
    let mb_y = body_y + 310.0;
    fill_round_rect(pm, px + 24.0, mb_y, prev_w - 48.0, 200.0, 14.0, rgba8(38, 38, 44, 240));
    let labels = [
        ("Size",    "612 KB"),
        ("Created", "Today  09:48"),
        ("Mods",    "Today  10:02"),
    ...
    fill_round_rect(pm, px + 24.0, cb_y, prev_w - 48.0, 280.0, 14.0, rgba8(38, 38, 44, 240));
    draw_text(pm, &fonts.sans_bold, "LIVE TRAINING", px + 40.0, cb_y + 30.0, 16.0, c(TEXT_SEC));
    draw_text(pm, &fonts.sans_bold, "loss",          px + 40.0, cb_y + 64.0, 22.0, c(TEXT_PRI));
    draw_text(pm, &fonts.sans, "1.342 -> 0.418",     px + 40.0, cb_y + 92.0, 20.0, c(SUCCESS));
    ...
    pm.fill_rect(rect(cx, cy - ch, cw, 1.0), &frame_paint, Transform::identity(), None);
    pm.fill_rect(rect(cx, cy,       cw, 1.0), &frame_paint, Transform::identity(), None);
    let pts: Vec<f32> = (0..60).map(|i| {
        let t = i as f32 / 59.0;
    ...
    if let Some(chart_path) = chart.finish() {
        ...
        cp.anti_alias = true;
        pm.stroke_path(&chart_path, &cp, &Stroke {
            width: 2.5, line_cap: LineCap::Round, line_join: LineJoin::Round, ..Default::default()
        }, Transform::identity(), None);
    }
}
...
fn draw_spotlight(pm: &mut Pixmap, fonts: &FontStack) {
    ...
    pm.fill_rect(rect(x + 20.0, y + 8.0, w - 40.0, 14.0), &hl, Transform::identity(), None);
    ...
    draw_text(pm, &fonts.sans_bold, "Q", inp_x + 28.0, inp_y + inp_h / 2.0 + 16.0, 38.0, c(TEXT_DIM));
    draw_text(pm, &fonts.sans, "stable diffusion xl", inp_x + 88.0, inp_y + inp_h / 2.0 + 17.0, 42.0, c(TEXT_PRI));
    ...
    // Result groups.
    struct Group<'a> { title: &'a str, rows: &'a [(&'a str, &'a str)], selected_idx: Option<usize> }
    let groups = [
        Group {
            title: "HUGGING FACE",
            rows: &[
                ("stabilityai/stable-diffusion-xl-base-1.0", "Text-to-Image  ·  9.4k stars"),
        ...
        Group {
            ...
            rows: &[
                ("sdxl-base-1.0.safetensors", "6.94 GB  ·  ~/models/sdxl"),
            ],
            selected_idx: None,
        },
        Group {
            title: "ARXIV",
            rows: &[
                ("SDXL: Improving Latent Diffusion Models for High-Resolution Synthesis", "2307.01952  ·  Podell et al."),
            ],
            selected_idx: None,
        },
    ];
    ...
    for g in &groups {
        draw_text(pm, &fonts.sans_bold, g.title, x + 50.0, ry, 16.0, c(TEXT_SEC));
        ry += 32.0;
        for (j, (a, b)) in g.rows.iter().enumerate() {
            let selected = g.selected_idx == Some(j);
            if selected {
                fill_round_rect(pm, x + 28.0, ry - 24.0, w - 56.0, 60.0, 10.0, rgba8(76, 134, 215, 230));
            }
            fill_round_rect(pm, x + 44.0, ry - 18.0, 44.0, 44.0, 10.0, rgba8(55, 55, 62, 240));
            draw_text(pm, &fonts.sans_bold, "H", x + 58.0, ry + 14.0, 30.0, c(ACCENT));
...
fn draw_notification(pm: &mut Pixmap, fonts: &FontStack) {
    ...
    draw_text(pm, &fonts.sans_bold, "✓", x + 36.0, y + 60.0, 38.0, rgba8(0, 0, 0, 220));
    draw_text(pm, &fonts.sans_bold, "Training complete", x + 96.0, y + 42.0, 22.0, c(TEXT_PRI));
    draw_text(pm, &fonts.sans, "llm-finetune  ·  step 3000  ·  loss 0.39", x + 96.0, y + 72.0, 18.0, c(TEXT_SEC));
    draw_text(pm, &fonts.sans, "now", x + w - 56.0, y + 42.0, 16.0, c(TEXT_DIM));
}
