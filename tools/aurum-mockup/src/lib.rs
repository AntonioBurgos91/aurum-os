...
#[inline] pub fn c(rgba: Rgba) -> Color {
    Color::from_rgba8(rgba[0], rgba[1], rgba[2], rgba[3])
}
#[inline] pub fn rgba8(r: u8, g: u8, b: u8, a: u8) -> Color {
...
#[inline] pub fn ca(rgba: Rgba, alpha: u8) -> Color {
...
pub fn stroke_round_rect(pm: &mut Pixmap, x: f32, y: f32, w: f32, h: f32, r: f32, color: Color, width: f32) {
    let r = r.min(w * 0.5).min(h * 0.5).max(0.0);
    let mut pb = PathBuilder::new();
    pb.move_to(x + r, y);
    pb.line_to(x + w - r, y);
...
pub fn drop_shadow(pm: &mut Pixmap, x: f32, y: f32, w: f32, h: f32, r: f32, intensity: u8) {
    for i in 0..8 {
        ...
        fill_round_rect(pm, x - g, y + g, w + 2.0 * g, h + 2.0 * g, r + g, rgba8(0, 0, 0, alpha));
    }
}
...
pub fn linear_gradient_2(pm: &mut Pixmap, x: f32, y: f32, w: f32, h: f32,
...
pub fn radial_vignette(pm: &mut Pixmap, w: u32, h: u32,
                        cx: f32, cy: f32, r0: f32, r1: f32,
    ...
    paint.shader = shader;
    paint.anti_alias = true;
    pm.fill_rect(rect(0.0, 0.0, w as f32, h as f32), &paint, Transform::identity(), None);
}
...
pub fn fill_round_rect_gradient(pm: &mut Pixmap,
...
pub fn load_fonts() -> Result<FontStack, String> {
    ...
    Ok(FontStack { sans, sans_bold, mono })
}

/// Blit `text` with `font` at (x, y) where y is the BASELINE. Returns the
/// width consumed so callers can chain glyphs.
pub fn draw_text(pm: &mut Pixmap, font: &Font, text: &str,
                  x: f32, y: f32, size: f32, color: Color) -> f32 {
    let mut pen_x = x;
    for ch in text.chars() {
        ...
        let (cr, cg, cb, ca) = (rgba.red() as u32, rgba.green() as u32, rgba.blue() as u32, rgba.alpha() as u32);
        ...
        for (i, &cov) in bitmap.iter().enumerate() {
            ...
            if dx < 0 || dy < 0 || dx >= pmw as i32 || dy >= pmh as i32 { continue; }
...
pub fn draw_glyph_centered(pm: &mut Pixmap, font: &Font, ch: char,
                            box_x: f32, box_w: f32, baseline_y: f32,
                            size: f32, color: Color) {
