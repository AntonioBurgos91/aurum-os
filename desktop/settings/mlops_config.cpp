...
void read_kv(const QString& path, QString& mlflowUri, QString& mlflowExp, QString& wandbKey,
             QString& wandbEntity) {
    QFile f(path);
...
void MlopsConfig::load() {
    ...
    m_wandbApiKey.clear();
    m_wandbEntity.clear();
    read_kv(config_path(), m_mlflowTrackingUri, m_mlflowExperiment, m_wandbApiKey, m_wandbEntity);
    emit changed();
}

bool MlopsConfig::reload() {
    load();
    return true;
}
